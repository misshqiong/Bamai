...
首次启动成功后，运行一次下面的命令，以后就再也不用手动启动 Bamai：

```bash
./bamai menubar on
```

它会构建并安装一个很小的菜单栏应用（需要 Xcode 命令行工具），登录自动出现并**守护服务**：自动启动 Bamai、异常退出自动拉起、手动停止后保持安静。圆点颜色对应 Mac 健康状态——绿/黄/红，服务未运行时为灰色；菜单里可以打开控制台、启动/重启/停止服务。如果项目位于 `~/Documents` 等 macOS 保护目录，应用首次访问项目时系统会弹一次授权框，点"允许"即可。

另一种方式是 `./bamai autostart on`，安装纯登录代理（带崩溃自愈），适合不想要菜单栏图标的场景。注意 launchd 无法读取保护目录（`~/Documents`、`~/Desktop`、`~/Downloads`）内的脚本，项目在这些位置时请优先使用菜单栏应用。
