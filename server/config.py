...
CAPTURES_DIR = Path(os.environ.get("BAMAI_CAPTURES_DIR", DATA_DIR / "captures")).expanduser()
...
APP_RETENTION_SECONDS = 48 * 60 * 60
...
# 连接级 nettop（不带 -P）实测约需 9 秒，超时须留足余量。
NETTOP_TIMEOUT_SECONDS = 20
...
# Ollama 闲置约 5 分钟会卸载模型，冷启动重新加载需 5–15 秒，30 秒窗口在冷启动叠加生成时不够用
HEALTH_EXPLAIN_TIMEOUT_SECONDS = 90
